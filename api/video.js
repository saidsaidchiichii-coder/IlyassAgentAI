...
async function handleVideoMain(req, res) {
  ...
  // 5. Pollinations Video
  if (!hasImage || provider === 'Pollinations') {
    try {
      const enc = encodeURIComponent(fullPrompt);
      const pHeaders = pollinationsKey ? { 'Authorization': `Bearer ${pollinationsKey}` } : {};
      const pVideoUrl = `https://image.pollinations.ai/prompt/${enc}?model=video&width=768&height=432&nologo=true&seed=${seed}`;
      const rV = await fetch(pVideoUrl, { headers: pHeaders, signal: AbortSignal.timeout(25000) }).catch(() => null);
      ...
      if (rV?.ok) {
        ...
        if (ct.includes('video') || ct.includes('mp4')) {
          ...
          if (buf.byteLength > 5000) {
            return res.status(200).json({
              success: true,
              videoBase64: `data:${ct};base64,${Buffer.from(buf).toString('base64')}`,
  ...
  if (hasImage && hfKey) {
    try {
      const r = await callHF('ali-vilab/i2vgen-xl', imageBase64 || imageUrl || fullPrompt, { prompt: fullPrompt, num_inference_steps: 20 });
      if (r.ok) {
        const buf = await r.arrayBuffer();
        if (buf.byteLength > 5000) {
          return res.status(200).json({
            success: true,
            videoBase64: `data:video/mp4;base64,${Buffer.from(buf).toString('base64')}`,
