...
async function handleDiscover(req) {
  ...
  const apiSources = [
    { name: 'MIXHUB',       type: 'llm',    models: ['gpt-4o', 'gpt-4-turbo', 'claude-3-5-sonnet'], endpoint: 'https://api.mixhub.com/v1/models', auth: mixhubKey ? `Bearer ${mixhubKey}` : null },
...
async function handleTranscribe(req, res) {
  ...
  if (!openaiKey) {
    return res.status(503).json({ 
      error: 'Transcription requires OPENAI_API_KEY in Vercel environment variables.' 
    });
  }
