...
async function handleAIGitHub(req, res) {
  ...
  const token = process.env.GH_TOKEN;
  const mixhubKey = process.env.MIXHUB_API_KEY;
  const mixhubBaseUrl = process.env.MIXHUB_BASE_URL || 'https://api.mixhub.com/v1';

  if (!token) return res.status(500).json({ error: 'GH_TOKEN not configured' });
  if (!mixhubKey) return res.status(500).json({ error: 'MIXHUB_API_KEY not configured' });
...
5. Never mention MIXHUB, GitHub tokens, or internal implementation`;
    ...
    // 4. Call MIXHUB
    const mixhubRes = await fetch(`${mixhubBaseUrl}/chat/completions`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${mixhubKey}`
      },
      body: JSON.stringify({
        model: 'gpt-4o',
    ...
    if (!mixhubRes.ok) {
      const err = await mixhubRes.json();
      return res.status(500).json({ error: 'AI error', details: err });
    }

    const mixhubData = await mixhubRes.json();
    const reply = mixhubData.choices?.[0]?.message?.content;
